...
Node v0.1 validates operator settings before binding sockets or starting miners, then validates incoming blocks by checking height, previous block hash, model transition metadata, configured sample count, trace-state continuity, trace root, commitment-chain structure, toy proof-of-work, and sampled deterministic training transitions. Valid blocks are tracked in a core fork-choice index, the active node view follows the best known branch, and reorg/known-block counters are exposed in node output. Accepted blocks are persisted as checksummed block-log records when `--data-dir` is set and gossiped to known peers; a `node.lock` file prevents two local nodes from writing the same data directory, shutdown waits for active inbound handlers before releasing that lock, and persistence failures are counted in node output instead of being ignored. New block logs include a checksummed chain-fingerprint header covering deterministic chain rules and configured chain parameters so startup fails clearly if local data belongs to a different rule set. Late peers request bounded peer lists and bounded headers after the first common locator hash before fetching missing best-chain blocks; header responses are screened for locator continuity, height sequence, configured sample count, and toy proof-of-work before full blocks are requested, fetched blocks must match those screened header hashes exactly, and malformed full-block sync responses are rejected before mutating fork choice. Oversized peer, locator, header, block, or inventory responses are rejected. Peer traffic is wrapped with protocol version, network id, and chain-fingerprint checks, startup HELLO announcements share a node's advertised listen address with configured peers, successful and failed HELLO announcements are counted, active inbound handlers and inbound response write failures are counted, inbound connection handlers are capped and joined during shutdown, inbound reads and outbound writes are bounded by configurable timeout and message-size limits, and the known-peer set is capped with rejected peer additions counted in node output. Non-dialable peer addresses such as port zero, unspecified IPs, multicast IPs, and IPv4 broadcast are rejected before entering the known-peer set. Nodes that bind to an unspecified IP such as `0.0.0.0` must set `--advertise` to a dialable address.
