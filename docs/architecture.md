...
8. Startup HELLO messages announce a node's dialable advertised listen address to configured peers so gossip can become bidirectional without manual peer entries on both sides. Nodes that bind to an unspecified IP must configure an explicit advertised address.
...
10. Peer messages and known peers are bounded by configured limits, inbound reads and outbound writes use configured timeouts, non-dialable peer addresses are rejected before retention, and malformed, oversized, incompatible, and rejected peer additions are counted in node snapshots.
