...
7. Accepted blocks are applied locally and gossiped to known peers.

## Current Research Limits

This is not a production cryptocurrency. RandomX is not integrated yet, the hash adapter is a labeled toy hash, there is no wallet, no persistence, no mempool, no fork-choice beyond next-block validation, and no adversarial networking hardening. The purpose of v0.1 is to make the core protocol loop executable and testable across actual local peers.
